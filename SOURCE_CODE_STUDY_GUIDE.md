...
这份文档写给不熟悉代码和 Git 的项目所有者。目标不是让你背代码，而是让你能回答三个问题：页面为什么这样设计、一次风险处理是怎么运行的、每项说法可以去哪里核实。

## 1. 先建立正确理解

SaaSGuide 不是一个训练大模型的项目，也不是已经上线的企业 SaaS。

它是一个本地运行的 AI 应用 Demo：
...
这是作者构造并标注的合成回归集，只能说明当前规则在这些固定案例上的表现。它不是 DeepSeek 准确率，也不是企业环境准确率。
